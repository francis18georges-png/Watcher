...
- Chaque nouveau domaine ou périmètre approuvé est inscrit dans `consents.jsonl` (JSONL signé) et persiste dans `policy.yaml` sous `domain_rules` (`domain` + `scope`). `allowlist_domains` reste synchronisé pour les garde-fous runtime existants. Aucune fenêtre popup récurrente : seule l'évolution de périmètre déclenche une nouvelle entrée.
