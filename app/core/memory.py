...
class Memory:
    ...
    def _init(self):
        con=sqlite3.connect(self.db_path); c=con.cursor()
        c.execute('CREATE TABLE IF NOT EXISTS items(id INTEGER PRIMARY KEY, kind TEXT, text TEXT, vec BLOB, ts REAL)')
        con.commit(); con.close()

    def add(self, kind: str, text: str):
        vec = embed_ollama([text])[0].astype('float32').tobytes()
        con=sqlite3.connect(self.db_path); c=con.cursor()
        c.execute('INSERT INTO items(kind,text,vec,ts) VALUES(?,?,?,?)',(kind,text,vec,time.time()))
        con.commit(); con.close()

    def search(self, query: str, top_k: int=8):
        q = embed_ollama([query])[0].astype('float32')
        con=sqlite3.connect(self.db_path); c=con.cursor()
        rows = c.execute('SELECT id,kind,text,vec FROM items').fetchall(); con.close()
        scored=[]
